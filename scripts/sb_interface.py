...
nz_pos = np.load(f'{nz_path}/nzs.npz')['nz_1']
nz_she = np.load(f'{nz_path}/nzs.npz')['nz_2']

plt.plot(z_nz, nz_pos[0, :], label='nz_pos[0]')
plt.plot(z_nz, nz_pos[1, :], label='nz_pos[1]')
plt.plot(z_nz, nz_she[0, :], label='nz_she[0]')
plt.plot(z_nz, nz_she[1, :], label='nz_she[1]')
plt.legend()

nz_pos_fmt = np.concatenate((z_nz[:, None], nz_pos.T), axis=1)
nz_she_fmt = np.concatenate((z_nz[:, None], nz_she.T), axis=1)
np.savetxt(f'{data_sb_path}/nz_pos_fmt.txt', nz_pos_fmt)
