...
for i in range(1, 100+1):
    ...
    if masked:
        heracles.write(path+mode+f"_sim_{i}/measured_cls_wmask.fits", _cls)
        heracles.write(path+mode+f"_sim_{i}/measured_cqs_wmask.fits", _cqs)
    else:
...
if masked:
    heracles.write(path+"mean_cls_wmask.fits", cls_m)
    heracles.write(path+"cov_cls_wmask.fits", cls_cov)
    heracles.write(path+"mean_cqs_wmask.fits", cqs_m)
    heracles.write(path+"cov_cqs_wmask.fits", cqs_cov)
else:
